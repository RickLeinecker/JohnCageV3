...
const NavBar = ({ userName, setterFunction, buttonList }: navBarData) => {
  const baseButtonList: string[] = ["Concerts", "About", "Calendar", "WebSocket", "WebSocketTest"];
      ...
      <Container>
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="me-auto">
            {baseButtonList.map((key, i) => (
              <NavButtons key={i} buttonName={key} />
            ))}
          </Nav>
          <Nav>
            {buttonList.map((key, i) => {
              if (key === "Logout") return LogoutDisplay(userName, setterFunction as Function);
